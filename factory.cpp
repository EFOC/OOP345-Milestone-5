		...
		for(auto &r : machineList){
			...
			if(r.getInstaller()){
				...
				for(size_t i = 0; i < j2.getItemSize(); i++){
					...
					if(r.getTaskName() == iptr->getItemInstallerTask()){
						j2.Installed(i, true);
						some = true;
						time++;
			...
			if(r.getRemover()){
				...
				string itemName = j2.getItemNumber(i);
				Item* iptr = im.FindItem(itemName);
				if(r.getTaskName() == iptr->getItemRemoverTask()){
...
int main(int argc, char* argv[])
{
        try {
                if (argc != 5){
                        throw string("Usage: ") + argv[0] + string(" csv-date-file csv=separator-character");
		}
