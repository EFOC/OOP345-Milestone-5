...
#include "util.h"
#include "t.h"
//implement functions from t.h here
...
void TaskManager::IntegrityCheck(){
                for(auto i : taskList){
                        if(!i.getTaskPass().empty() && FindTask(i.getTaskPass()) == nullptr){
                                throw string("Item '") + i.getTaskName() + "' references installer task '" + i.getTaskPass() + "' but it does not exist!";
                        }
                        if(!i.getTaskfail().empty() && FindTask(i.getTaskfail()) == nullptr){
                                throw string("Item '") + i.getTaskName() + "' references remover task '" + i.getTaskfail() + "' but it does not exist!";
                        }
                }
        }

Task* TaskManager::FindTask(std::string& name){
                for(size_t i = 0; taskList.size(); i++){
